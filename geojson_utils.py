...
def masks_to_annotation(datasets_dir, save_path):
    ...
    if os.path.exists(masks_dir):
        ...
        for file in [f for f in os.listdir(masks_dir)]:
            ...
            mask_img = imageio.imread(file_full)
            ...
            io.imageio(os.path.join(sample_path, "mask_labels.png"), mask_img)
...
def gen_mask_from_geojson(files_proc, masks_to_create_value=['filled', 'edge', 'distance', 'weigthed', 'border_mask'], img_size=None, scale=1.0, infer=False):
    masks_to_create = {}
    ...
    for i, file_proc in enumerate(files_proc):
        ...
        for roi_key, roi in annot_dict_all.items():
            roi['pos'] = roi['pos']*scale
